    ...
    <div className="w-full max-w-6xl mx-auto mt-16 mb-16 p-8 bg-gradient-to-r from-black/70 to-[#171717] rounded-lg space-y-8 border-t border-b border-[#2a2938]">
      ...
      <div className="flex flex-wrap justify-center gap-8 items-stretch">
        {pricingOptions.map((option, index) => (
          <div
            key={index}
            className={`flex flex-col border-2 border-opacity-50 border-gray-700 rounded-lg p-6 w-full md:w-1/3 lg:w-1/4 hover:shadow-2xl transform hover:scale-105 transition-all duration-300 ease-in-out shadow-md ${option.bgColor}`}
          >
            ...
            <div className="mt-10 text-center">
              <Link href="/login">
                <Button className="w-3/4 bg-[#646FD4] hover:bg-[#5a62d4] text-white font-semibold py-2 px-4 rounded-lg shadow-lg transition-colors duration-300 ease-in-out">
                  {option.buttonText}
                </Button>
